...
class SUMO_Subscription_PaymentGateways {
	...
	/**
	 * Get subscription supported inbuilt payment gateways.
	 * 
	 * @return array
	 */
	public static function get_inbuilt_subscription_payment_gateways() {
		return self::$subscription_payment_gateways ;
	}
		...
		if ( self::$checkout_has_subscription ) {
			if ( ! self::$mixed_payment_gateways_enabled ) {
				if ( self::$auto_payment_gateways_enabled ) {
					...
					if ( self::PAYPAL_SUBSCRIPTIONS === $gateway_id ) {
						if ( WC()->cart->cart_contents && 1 !== count( WC()->cart->cart_contents ) ) {
							$need = true ;
						}
					}
				}
			...
			if ( self::$hide_auto_payment_gateways_when_non_subscriptions_in_cart && in_array( $gateway_id, self::get_inbuilt_subscription_payment_gateways() ) ) {
				$need = false ;
			}
		}
